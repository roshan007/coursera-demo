...
class CourseMixin(object):
    ...
    def _copyf(self, datalist, key):
        list = []
        for item in datalist:
            if key in item:
                timestamp = self._get_datetime_from_epoch(item[key])
                if timestamp.year == datetime.today().year and timestamp.month == datetime.today().month:
    ...
    def _utilize_coursera_api(self, id):
        ...
        result['courses'] = self._copyf(result['elements'], 'startDate')
    ...
    def _get_instructor_response(self, id):
        if id is None:
            return {'elements': ''}
        url = 'https://api.coursera.org/api/instructors.v1/'+id+'?includes=universities&fields=photo,bio,fullName,title,department,website,websiteTwitter,websiteFacebook,websiteLinkedin,websiteGplus'
        return self._utilize_url(url)
...
class CourseListApi(CourseMixin, generics.GenericAPIView):
    ...
    def get(self, request):
        courses = self._utilize_coursera_api(None)
...
class CourseDetailApi(CourseMixin, generics.GenericAPIView):
    ...
    def get(self, request, id):
        course = self._utilize_coursera_api(id)
